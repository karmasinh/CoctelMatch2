...
exports.addNewUser = async (req, res, next) => {
  try {
    const { name, email, password, city, gender, bio, role: requestedRole } = req.body;
    ...
    if (existingUser) {
      ...
      bcrypt.hash(password, 8, async function (err, hash) {
        ...
        if (err) {
          ...
          // Create a new user
          // Determine role safely: allow only 'user' or 'mixologist' at signup
          const allowedSignupRoles = ["user", "mixologist"];
          const safeRole = allowedSignupRoles.includes((requestedRole || "").toLowerCase())
            ? (requestedRole || "user").toLowerCase()
            : "user";
