...
exports.getAllRecipe = async (req, res, next) => {
  try {
    ...
    if (flavors) {
      try {
        const arr = Array.isArray(flavors) ? flavors : JSON.parse(flavors);
        if (Array.isArray(arr) && arr.length > 0) {
          filter.flavors = { $in: arr };
        }
      } catch (e) {
        // Allow comma-separated fallback
        if (typeof flavors === "string" && flavors.includes(",")) {
          const arr = flavors.split(",").map((s) => s.trim()).filter(Boolean);
          if (arr.length > 0) filter.flavors = { $in: arr };
        }
      }
    }
